...
def play_music():
    ...
    if not paused:
        pygame.mixer.music.load(os.path.join(root.directory, current_song))  # Загружаем музыкальный файл
        pygame.mixer.music.play()  # Проигрываем музыку
    else:
        pygame.mixer.music.unpause()  # Продолжаем воспроизведение музыки
        paused = False
def pause_music():
    global paused
...
menubar.add_cascade(label="Orginise", menu=organise_menu)
